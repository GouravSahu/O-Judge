...
def finalpage(request,question_id,form_id):
    ...
    subprocess.run("sudo docker cp "+cmd+" my-gcc:/gourav.cpp",shell=True)

    input="media/"+str(question_id)+"/tests.txt"
    subprocess.run("sudo docker cp "+input+" my-gcc:/tests.txt",shell=True)

    output="media/out.txt"
    subprocess.run("sudo docker cp "+output+" my-gcc:/out.txt",shell=True)

    subprocess.run("sudo docker exec my-gcc g++ gourav.cpp ./a.out",shell=True)

    command= 'sudo docker exec -ti my-gcc sh -c "./a.out <tests.txt > out.txt"'

    subprocess.run(command,shell=True)
    
    subprocess.run("sudo docker exec my-gcc rm -rf a.out",shell=True)

    command2= "sudo docker cp my-gcc:out.txt " +"media/"+str(question_id)+"/out.txt"
